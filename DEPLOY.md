...
- **`GET /api/shopify/auth`**: el front hace **navegación directa** (`window.location`) hacia la API. El JWT de sesión va en query **`kovo_token`** (un GET del navegador no puede enviar `Authorization`). El servidor responde siempre con **302** a Shopify (o a login/`canales?shopify=error` si falta token o configuración). **`SHOPIFY_REDIRECT_URI`** en el servidor debe coincidir **exactamente** con la URL permitida en Partner Dashboard (p. ej. `https://kovo.services/api/shopify/callback`).
